	...
	do _g.app.APIClient:getInstance():getTasks(function(response)
		...
		do
			...
			if not (records ~= nil) or records:getSize() < 1 then
				do self.list:addWidget1(_g.jk.widget.AlignWidget:forWidget(self.context, _g.jk.widget.LabelWidget:forText(self.context, "No record"), 0.5, 0.5, 0), 1.0) end
			else
				...
				if array ~= nil then
					...
					do
						...
						while n < m do
							...
							if record ~= nil then
								local task = _g.jk.lang.String:safeString(record:getString("name", nil)) .. " - " .. _g.jk.lang.String:safeString(record:getString("desciption", nil))
...
function app.Tasklist:createWidget()
	...
	do
		...
		do self.list:setWidgetMargin(self.context:getHeightValue("5mn")) end
		do self.list:setWidgetSpacing(self.context:getHeightValue("5mn")) end
...
function app.APIClient:create(context, parentWidget)
	...
	do _g.app.APIClient.instance:setApiUrl("http://ec2-54-169-104-179.ap-southeast-1.compute.amazonaws.com:30123") end
